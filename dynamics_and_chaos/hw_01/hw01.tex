...
\item 2.2.8 \\
\textit{Solution:} \\
Based on the given flow diagram $\dot x = x(x + 1)^2(x - 2)$. Now to verify that this satisfies the criteria we are given I will plug in values in the ranges given and verify they match the expected flow direction.
...
Looking at Figure \ref{fig:f5} makes it obvious that $x^* = 0$ is unstable and $x^* = \frac {k_1 a}{k_{-1}}$ is stable. \textbf{TODO:} Insert actual image. \\
